...
class MLPPolicy(BasePolicy):
    ...
    def build_action_sampling(self):
        if self.discrete:
            ...
            # logits_na: [N, num_action_classes]
            self.sample_ac = tf.squeeze(tf.multinomial(logits_na, num_samples=1), axis=1) # [N,]
        else:
    ...
    def build_baseline_forward_pass(self):
        self.baseline_prediction = tf.squeeze(build_mlp(self.observations_pl, output_size=1, scope='nn_baseline', n_layers=self.n_layers, size=self.size)) # (N,)
...
class MLPPolicyPG(MLPPolicy):
    ...
    def update(self, observations, acs_na, adv_n=None, acs_labels_na=None, qvals=None):
        ...
        if self.nn_baseline:
            # value function的目标是normalizaed Q function，应该就是因为只sample一个样本
            # 等于说先sample a_t，再sample一整条trajectory计算value函数
            # 就是actor-critic里面的蒙塔卡罗估计value函数的方法
            ...
            _, baseline_loss = self.sess.run([self.baseline_update_op, self.baseline_loss], feed_dict={self.observations_pl: observations, self.targets_n: targets_n})
            print('\nBaseline loss: {}'.format(baseline_loss))
        return loss
