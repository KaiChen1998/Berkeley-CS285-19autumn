...
class PGAgent(BaseAgent):
    ...
    def train(self, obs, acs, rews_list, next_obs, terminals):
        ...
        """
            ...
            ----------------------------------------------------------------------------------
            obs: [N, ob_dim], acs: [N,], next_obs: [N, ob_dim], terminals: [N,], rews_list: list
        """
        ...
        if not self.use_gae:
            ...
            rews_concat = np.concatenate(rews_list) # [N,]
            b_t_unnormalized = self.actor.run_baseline_prediction(obs)
            b_tp_unnormalized = self.actor.run_baseline_prediction(next_obs)
            b_t = b_t_unnormalized * np.std(rews_concat) + np.mean(rews_concat)
            b_tp = b_tp_unnormalized * np.std(rews_concat) + np.mean(rews_concat)
            delta = rews_concat + (1 - terminals) * self.gamma * b_tp - b_t

            # step 2: calculate advantage
            advantage_values = self.estimate_advantage_gae(delta, rews_list)
            q_values = advantage_values + b_t
    ...
    def estimate_advantage_gae(self, delta, rews_list):
        # delta: [N,], rews_list: list of list of scalar
        adv_n = []
        global_index = 0
        
        for i in range(len(rews_list)):
            delta_list = delta[global_index: global_index + len(rews_list[i])]
            global_index += len(rews_list[i])

            for j in range(len(delta_list)):
                
                # 1) create a list of indices (t'): goes from t to T-1
                indices = np.arange(len(delta_list) - j)

                # 2) create a list where the entry at each index (t') is gamma^(t'-t)
                discounts = np.power(self.gamma * self.lam, indices)
                ...
                discounted_rtg = discounts * delta_list[j:]
                ...
                adv_n.append(sum_discounted_rtg)
    ...
    def _discounted_cumsum(self, rewards):
        ...
        for start_time_index in range(len(rewards)): 
            ...
            # 1) create a list of indices (t'): goes from t to T-1
            indices = np.arange(len(rewards) - start_time_index)
