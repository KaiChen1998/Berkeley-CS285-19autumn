...
class MLPPolicy(BasePolicy):
    ...
    def __init__(self,
        ...
        **kwargs):
        ...
        self.policy_vars = [v for v in tf.all_variables() if policy_scope in v.name and 'train' not in v.name] #train_op
        self.policy_saver = tf.train.Saver(self.policy_vars, max_to_keep=None) # None表示不指定，都保存
    ...
    def build_action_sampling(self):
        ...
        # 单位协方差矩阵
        self.sample_ac = mean + tf.exp(logstd) * tf.random_normal(tf.shape(mean), 0, 1) # 之前在VAE中见到过类似的操作
    ...
    def get_action(self, obs):
        ...
        if len(obs.shape)>1:
            ...
            observation = obs[None] # vector 2 matrix
...
class MLPPolicySL(MLPPolicy):
    ...
    def update(self, observations, actions):
        ...
        batch_loss, _ = self.sess.run([self.loss, self.train_op], feed_dict={self.observations_pl: observations, self.acs_labels_na: actions})
