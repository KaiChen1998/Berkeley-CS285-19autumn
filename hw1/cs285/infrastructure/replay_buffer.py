...
class ReplayBuffer(object):
    ...
    def add_rollouts(self, paths, concat_rew=True):
        ...
        # replay_buffer是累计所有的数据啊，数据集是越来越大的，包括expert data和标好的data
        for path in paths:
            self.paths.append(path)

        # convert new rollouts into their component arrays, and append them onto our arrays
        observations, actions, rewards, next_observations, terminals = convert_listofrollouts(paths, concat_rew) # [batch_size, dim]
    ...
    def sample_random_data(self, batch_size):
        ...
        index = np.random.permutation(self.obs.shape[0]) < batch_size
        return self.obs[index], self.acs[index], self.rews[index], self.next_obs[index], self.terminals[index]
