...
class ACAgent(BaseAgent):
    def __init__(self, sess, env, agent_params):
        ...
        self.actor = MLPPolicyAC(sess, 
                               ...
                               ) # now it only accounts for action prediction
    ...
    def estimate_advantage(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        adv_n = re_n + (1 - terminal_n) * self.gamma * self.critic.forward(next_ob_no) - self.critic.forward(ob_no)
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        for i in range(self.agent_params['num_critic_updates_per_agent_update']):
            # 其中已经涉及到argmin所带来的多步迭代了，其实这个很鸡肋
