...
class DQNAgent(object):
    def __init__(self, sess, env, agent_params):
        ...
        self.batch_size = agent_params['batch_size'] # collect batch size from dqn_utils.py, but 恕我直言按你的写法最后用的是给定的batch size
        ...
        if(self.agent_params['use_polyak']):
            self.target_update_freq = 1

        self.replay_buffer_idx = None # 当前数据的index
        self.exploration = agent_params['exploration_schedule'] # collect policy
        self.optimizer_spec = agent_params['optimizer_spec']

        self.critic = DQNCritic(sess, agent_params, self.optimizer_spec)
        self.actor = ArgMaxPolicy(sess, self.critic) # actor像是搭建在critic之上的，只是用了critic的q_value求了argmax
    ...
    def step_env(self):
        ...
        if perform_random_action:
            action = np.random.randint(self.num_actions) # discrete action space by default
        else:
            ...
            enc_last_obs = enc_last_obs[None, :] # [1, N, dim]
        ...
        self.replay_buffer.store_effect(self.replay_buffer_idx, action, reward, done) # 前面存了obs，现在存结果
