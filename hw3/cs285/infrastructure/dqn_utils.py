...
def get_env_kwargs(env_name):
    if env_name == 'PongNoFrameskip-v4':
        ...
        kwargs = {
            ...
            'batch_size': 32,
            'lander': True,
            'optimizer_spec': lander_optimizer(), # 最普通的adams, lr = 1e-3
        }
...
# optimizer: ["constructor", "kwargs", "lr_schedule"]
# 所以就是指定了constructor和lr_schedule，其他的都自己加
# lr_schedule产生lr之后作为placeholder输入
...
def lander_exploration_schedule(num_timesteps):
    # 总运行时间的前10%，epsilon值从1降为0.02
    # num_step： total iteration times
    return PiecewiseSchedule(
        [
            (0, 1),
...
class PiecewiseSchedule(object):
    def __init__(self, endpoints, interpolation=linear_interpolation, outside_value=None):
        ...
        outside_value: float, 即超出指定范围的时候应该怎么办
...
class MemoryOptimizedReplayBuffer(object):
    ...
    def sample(self, batch_size):
        ...
        idxes = sample_n_unique(lambda: random.randint(0, self.num_in_buffer - 2), batch_size) # Why -2 ?
    ...
    def _encode_observation(self, idx):
        # Not only take single frame
        # But also take ajancent frames to concentrate
        ...
        if start_idx < 0 or missing_context > 0:
            frames = [np.zeros_like(self.obs[0]) for _ in range(missing_context)] # 缺多少补几张0
            ...
            return self.obs[start_idx:end_idx].transpose(1, 2, 0, 3).reshape(img_h, img_w, -1) # [H, W, N * C]
    ...
    def store_frame(self, frame):
        ...
        ret = self.next_idx
        self.next_idx = (self.next_idx + 1) % self.size # FIFO
