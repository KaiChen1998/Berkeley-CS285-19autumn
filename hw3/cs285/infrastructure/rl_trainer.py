...
class RL_Trainer(object):
    ...
    def __init__(self, params):
        ...
        if 'env_wrappers' in self.params:
            ...
            self.env = wrappers.Monitor(self.env, os.path.join(self.params['logdir'], "gym"), force=True) # 打包环境，记录相关结果于该文件夹当中
            self.env = params['env_wrappers'](self.env) # some predefined operation for atari
            
            self.mean_episode_reward = -float('nan') # original value for mean_episode_reward
            self.best_mean_episode_reward = -float('inf') # original value for best resutl
        self.env.seed(seed)

        # Maximum length for episodes
        self.params['ep_len'] = self.params['ep_len'] or self.env.spec.max_episode_steps
        MAX_VIDEO_LEN = self.params['ep_len'] # overwrite

        # Is this env continuous, or self.discrete?
        discrete = isinstance(self.env.action_space, gym.spaces.Discrete) # action space discrete or not
        # Are the observations images?
        img = len(self.env.observation_space.shape) > 2 # observation space

        self.params['agent_params']['discrete'] = discrete

        # Observation and action sizes

        ob_dim = self.env.observation_space.shape if img else self.env.observation_space.shape[0] # [H, W, C] or scalar
        ac_dim = self.env.action_space.n if discrete else self.env.action_space.shape[0] # scalar
    ...
    def run_training_loop(self, n_iter, collect_policy, eval_policy,
        ...
        for itr in range(n_iter):
            ...
            elif itr % self.params['scalar_log_freq'] == 0 or itr + 1 == n_iter:
    ...
    def collect_training_trajectories(self, itr, load_initial_expertdata, collect_policy, batch_size):
        ...
        if(itr == 0) and load_initial_expertdata:
            print("\nCollecting expert data from {}...".format(load_initial_expertdata))
        ...
        train_video_paths = None
        if self.logvideo: # 肯定是不一定一样的，但是是同一个policy产生的结果，所以可以类比
    ...
    def train_agent(self):
        ...
        for train_step in range(self.params['num_agent_train_steps_per_iter']):
            ...
            self.batch_loss = self.agent.train(ob_batch, ac_batch, re_batch, next_ob_batch, terminal_batch)
            # print('\nBatch {}: loss: {}'.format(train_step, batch_loss))
    ...
    def perform_dqn_logging(self):
        episode_rewards = get_wrapper_by_name(self.env, "Monitor").get_episode_rewards() # Monitor应该是有纪录的
        ...
        logs["Train_EnvstepsSoFar"] = self.agent.t # 迭代次数
        print("Timestep %d" % (self.agent.t,))
        
        print("Current loss {}".format(self.batch_loss))
        logs["Training loss"] = self.batch_loss
