    ...
    def update(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        for i in range(self.num_grad_steps_per_target_update * self.num_target_updates):
            if i % self.num_grad_steps_per_target_update == 0:
                targets_n = re_n + (1 - terminal_n) * self.gamma * self.forward(next_ob_no)

            loss, _ = self.sess.run([self.critic_loss, self.critic_update_op], feed_dict = {self.sy_ob_no:ob_no, self.sy_target_n: targets_n})
