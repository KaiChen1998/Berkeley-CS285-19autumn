...
class FFModel(BaseModel):
    ...
    def define_forward_pass(self):
        # Pipeline: normalize -> forward -> unnormalize + fix offset
        ...
        self.next_obs_pred = self.obs_pl + self.delta_pred_unnormalized # TODO(Q1) Predict next observation using current observation and delta prediction (not that next_obs here is unnormalized)
    ...
    def statistics_to_dict(self, data_statistics):
        return {
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        feed_dict = {self.obs_pl: observations, self.acs_pl: actions}
        feed_dict.update(self.statistics_to_dict(data_statistics))
        return self.sess.run(self.next_obs_pred, feed_dict = feed_dict) # TODO(Q1) Run model prediction on the given batch of data

    def update(self, observations, actions, next_observations, data_statistics):
        # train the model
        feed_dict = {self.obs_pl: observations, self.acs_pl: actions, self.delta_labels: next_observations - observations}
        feed_dict.update(self.statistics_to_dict(data_statistics))
        _, loss = self.sess.run([self.train_op, self.loss], feed_dict = feed_dict) # TODO(Q1) Run the defined train_op here, and also return the loss being optimized (on this batch of data)
        return loss